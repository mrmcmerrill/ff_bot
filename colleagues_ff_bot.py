...
def random_init():
    ...
    phraseTwo = '. ' + random_name()[0] + ' you\'re a pussy.'
    phraseThree = '. ' + random_name()[0] + ' you DONT KNOW FANTASY.'

    phrases = [phraseOne, phraseTwo, '. Jess even gonna let you wear the dress, Luke?',
               '. Will, probably should let Lyss have a crack at it at this point, right? What do you have to lose?',
               '. Con...Con...Conner...hello...are you there?', phraseThree,
               '. Robert, aka pussy. You were so worried about your score on the Wonderlic, you would rather take a 0...You think you can avoid it every year?',
               '. Corey, how much did that 13-1 season pay out?', '. Greg, could you point me in the direction of the nearest P Fit?',
               '. DOLF, wouldn\'t it just be easier if you just dated Kayla? Get it off your chest, we all support you.', '. Nick, ?',
               '. Ben, can I get a minute of your precious time? I promise it will only take a minute to check out this link...www.google.com/search?q=how-to-use-the-waiver-wire.',
               '. Sott, Mrs. B called...Emma is waiting for you at the crib.']
    
    joke = ''
    joke = random.choice(phrases)
...
#        league = League(league_id=league_id, year=year, username=espn_username, password=espn_password)
    ...
    if function=="get_matchups":
        ...
        try:
            joke = random_init()
            text = salutation + os.environ["INIT_MSG"] + joke
        except KeyError:
